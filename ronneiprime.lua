-- ==============================================================================
--  RONNEI HUB - LUXURY FRAMEWORK WITH EMBEDDED LENNON HUB
...
-- ==================== 4. SIDEBAR VÀ VÙNG CHỨA NỘI DUNG ====================
...
-- Khung chứa chính dành riêng để nhúng Lennon Hub
local LennonHostPage = Instance.new("Frame", ContentArea)
LennonHostPage.Name = "LennonHostPage"
LennonHostPage.Size = UDim2.new(1, 0, 1, 0)
LennonHostPage.BackgroundTransparency = 1
LennonHostPage.Visible = true

-- Tab bấm trên Sidebar
...
-- ==================== 5. CHẠY LENNON HUB & TỰ ĐỘNG NHÚNG ====================
task.spawn(function()
    pcall(function()
...
-- Vòng lặp giám sát: Triệt tiêu nút rác & nhúng Frame Lennon vào Ronnei Hub
task.spawn(function()
                ...
                for _, gui in ipairs(container:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui ~= ScreenGui then
                        for _, desc in ipairs(gui:GetDescendants()) do
                            -- 1. Triệt tiêu các nút trôi nổi ngoài màn hình
                            if desc:IsA("GuiObject") then
                                local t = (desc:IsA("TextLabel") or desc:IsA("TextButton")) and desc.Text:upper() or ""
                                local n = desc.Name:lower()
                                if t:find("RESET GUI") or (t == "≡" and desc.AbsoluteSize.X < 65) or n:find("logo") then
                                    desc.Visible = false
                                    desc.Position = UDim2.new(50, 0, 50, 0)
                                end
                            end

                            -- 2. Tìm khung cửa sổ chính của Lennon Hub và nhúng vào Ronnei Hub
                            if not embedded and desc:IsA("Frame") and desc.Parent and desc.Parent:IsA("ScreenGui") then
                                local isLennonBox = false
                                for _, sub in ipairs(desc:GetDescendants()) do
                                    if sub:IsA("TextLabel") and (sub.Text:find("LENNON") or sub.Text:find("TOP 4") or sub.Text:find("TELEPORT") or sub.Text:find("BEST EGG")) then
                                        isLennonBox = true
                                        break
                                    end
                                end
                                ...
                                if isLennonBox then
                                    ...
                                    -- Làm mềm viền xanh của Lennon cho hòa hợp với Ronnei Hub
