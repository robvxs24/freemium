-- ==============================================================================
--  RONNEI HUB - STEAL AN EGG (OFFICIAL V1.3 - INSTANT HOST HIDE WATCHDOG)
--  Tính năng mới: Tự động phát hiện và quăng menu script gốc ra khỏi màn hình khi khởi động
-- ==============================================================================
...
-- ==================== STARTUP WATCHDOG: ĐÓNG / QUĂNG MENU GỐC NGAY LẬP TỨC ====================
local originalEquinozBtn = nil

local function instantHideHostMenu(inst)
    pcall(function()
        ...
        if inst:IsA("TextLabel") or inst:IsA("TextButton") then
            ...
            if txt:find("EQUINOZ") or txt:find("HORIZON") or txt:find("VEUURTUMWE") or txt:find("ANTI HIT") then
                ...
                -- Tìm khung chứa chính của menu gốc và quăng ra ngoài màn hình hoặc gọi nút Close
                ...
                if mainFrame and not mainFrame:IsDescendantOf(ScreenGui) then
                    mainFrame.Position = UDim2.new(50, 0, 50, 0)
                    mainFrame.Visible = false
                end
            end

            -- Nếu phát hiện nút Close của menu gốc, tự động kích hoạt để đóng ngay
            if inst:IsA("TextButton") and (txt == "X" or txt == "CLOSE") and not inst:IsDescendantOf(ScreenGui) then
...
for _, c in ipairs({CoreGuiService, gethui and gethui(), LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui")}) do
    if c then
        for _, desc in ipairs(c:GetDescendants()) do instantHideHostMenu(desc) end
        c.DescendantAdded:Connect(instantHideHostMenu)
    end
end
            ...
            if c then
                for _, gui in ipairs(c:GetChildren()) do
                    if gui ~= ScreenGui and gui:IsA("ScreenGui") then
                        ...
                        if name:find("equinoz") or name:find("horizon") or name:find("hh") then
                            gui.Enabled = false
